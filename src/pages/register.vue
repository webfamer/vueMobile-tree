  ...
  <div class="personal">
    <div class="personal-c">
      ...
      <div class="main">
        ...
        <div class="main-head">
          <span>输入您的手机号</span>
          <span @click="toLogin">已有账号登陆</span>
        </div>
        <div class="form">
          <Input v-model="registInfo.phone" placeholder="手机号" />
          <Input v-model="registInfo.password" placeholder="密码" type="password" password/>
          <Input v-model="registInfo.verifyCode" placeholder="验证码">
            <span slot="suffix" @click="getVerifyCode">获取验证码</span>
          </Input>
          <Input v-model="registInfo.code" placeholder="邀请码" />
        </div>
        <div class="btn">
          <Button type="success" long @click="registerCount">注册</Button>
          <!-- <div>
            <span class="clickRegister">没有账号？点击立即注册</span>
          </div> -->
        </div>
        <div class="tel">
...
export default {
  ...
  methods: {
    registerCount() {
      //注册账户
      api.register({...this.registInfo}).then(res=>{
        if(res.code===1){
